...
- [x] **NATS Services API** — در `Dext.Net.Nats.Services.pas`: `AddService` / `AddEndpoint` / `AddGroup` (`TDextNatsServiceGroup`، prefix تو در تو + queue inherit/override) / `Stop` / `Reset`، پاسخ خودکار `$SRV.PING|INFO|STATS` (all/kind/instance)، queue پیش‌فرض `q`، `Respond` / `RespondError`، `DoneHandler` / `ErrorHandler` (`TNatsServiceConfig`)، Stop = UNSUB SIDs سرویس + Flush بهترین‌تلاش (بدون `client.Drain` / disconnect). **Gaps vs nats.go micro:** `StatsHandler` / schema JSON عمیق، auto-stop روی connection closed، per-sub `Drain` (کلاینت ندارد؛ Unsubscribe+Flush)، metadata immutability سخت‌گیرانه
