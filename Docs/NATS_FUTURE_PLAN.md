...
- [ ] **Architecture V2 Phase 7 leftovers** — landed: RecvLoop `Receive(TByteSpan)`, pooled encode scratch, span header codec, JS admin `TBytes` parse, SIMD `IndexOfCrLf`. Still skipped: borrowed-span MSG callback (breaking), vectored TCP write (no Dext `writev`), FastPath. Still open: owned-vs-borrowed baselines after a real compile/bench run. Façade still duplicates Streams/Consumers/Push/Ordered (Fetch reuses owned fetcher).
