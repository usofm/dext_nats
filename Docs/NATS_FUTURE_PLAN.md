...
- [ ] **Shutdown / WSAEINTR 10004** — در working tree هست (`ReceiveBytes` وقتی `FClosing`؛ بلعیدن خطای teardown؛ تست `Disconnect_ShouldJoinThreadsCleanly`) ولی **هنوز commit نشده**. باید قبل از ریلیز merge شود.
- [ ] **Stream compression + placement** — `TNatsStoreCompression` / `TNatsPlacement` روی `TNatsStreamConfig` و map از Object Store config؛ کد + تست در working tree؛ **هنوز commit نشده**. تا commit، در `AGENTS.md` همچنان deferred محسوب می‌شود.
...
2. **P0 §۳** committed باشد: shutdown 10004 + compression/placement (+ تست‌های unit/JS مربوط).
