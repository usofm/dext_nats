...
- [ ] **Architecture V2 Phase 7 leftovers** — landed: RecvLoop `Receive(TByteSpan)`, pooled encode scratch, span header codec, JS admin `TBytes` parse, SIMD `IndexOfCrLf`. Debug Win32 compile + Parser/Protocol/Internal/JS-JSON unit tests passed. Live on nats-server v2.14.5: NestedFetch 50×, OrderedConsumer, PurgeDeletes. Still skipped: borrowed-span MSG callback (breaking), vectored TCP write (no Dext `writev`), FastPath. Still open: owned-vs-borrowed baselines (`DEXT_NATS_RUN_BENCH=1`). Façade still duplicates Streams/Consumers/Push/Ordered (Fetch reuses owned fetcher).
